...
const fieldClass =
  'min-h-11 w-full rounded-[10px] border border-[#2f2226] bg-[#181114] px-4 text-[0.88rem] text-[#f2eaeb] outline-none transition-colors placeholder:text-[#6e5658] focus:border-[#8a2c34]';
const oauthBtn =
  'flex min-h-11 cursor-pointer items-center justify-center gap-2.5 rounded-[10px] border border-[#2f2226] bg-[#181114] text-[0.84rem] font-[600] text-[#f2eaeb] transition-colors hover:border-[#5e3a3e] hover:bg-[#1d1518] disabled:cursor-not-allowed disabled:opacity-60';
...
export function LoginPage() {
  ...
  return (
    <main className="grid h-[100svh] grid-cols-2 overflow-hidden bg-[#100b0c] font-['Inter',system-ui,sans-serif] text-[#f2eaeb] max-[820px]:grid-cols-1">
      {/* left — background image with logo top-left */}
      <div className="relative overflow-hidden border-r border-[#2b1f22] max-[820px]:hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-10">
          <p className="max-w-[320px] text-[1.25rem] font-[620] leading-[1.25] text-white">
      ...
      {/* right — form */}
      <div className="relative flex flex-col justify-center overflow-y-auto px-[8%] py-8 max-[1100px]:px-[6%] max-[820px]:px-6">
        {/* logo top-left on mobile (image panel hidden) */}
        <div className="absolute left-6 top-6 hidden max-[820px]:block">
          <BrandMark />
        </div>

        <div className="mx-auto flex w-full max-w-[400px] flex-col gap-5 max-[820px]:mt-12">
          <div className="flex flex-col gap-1.5 text-center">
            <h1 className="text-[2.1rem] font-[760] leading-[1.05] tracking-[-0.01em] text-[#f2eaeb] max-[640px]:text-[1.8rem]">
              {mode === 'signin' ? 'Welcome Back' : 'Create account'}
            </h1>
            <p className="text-[0.84rem] text-[#a89799]">
              {mode === 'signin'
          ...
          <form className="flex flex-col gap-4" onSubmit={submit}>
            <label className="flex flex-col gap-1.5">
              <span className="text-[0.82rem] font-[620] text-[#f2eaeb]">Your email</span>
              <input
                type="email"
                required
                value={email}
            ...
            <label className="flex flex-col gap-1.5">
              <span className="text-[0.82rem] font-[620] text-[#f2eaeb]">Password</span>
            ...
            <div className="flex items-center justify-between">
              <label className="flex cursor-pointer items-center gap-2 text-[0.82rem] text-[#a89799]">
                <input
                  type="checkbox"
                  checked={remember}
              ...
              <button
                type="button"
                onClick={forgot}
                className="text-[0.82rem] font-[600] text-[#d98a90] transition-colors hover:text-[#f0b9bd]"
              >
                Forgot password?
              </button>
            </div>
            ...
            <button
              ...
              className="flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-[10px] bg-[#77262d] text-[0.9rem] font-[700] text-white transition-colors hover:bg-[#8a2c34] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading === 'form' ? <Loader2 size={17} className="animate-spin" /> : null}
              {mode === 'signin' ? 'Sign In' : 'Create account'}
            </button>
          </form>

          <p className="text-center text-[0.84rem] text-[#a89799]">
